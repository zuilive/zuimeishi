...
// }
    ...
    public function showfile(){
        $handler = opendir(C("DB_BACKUP"));  //打开备份目录
        $i = 1;
        while (($filename = readdir($handler))!==false) {   //循环获得文件名，使用!==因为某些文件名有叫'0'的就会结束
            if ($filename != "." && $filename != ".." && $filename != "index.html") {    //去除目录下名字为'.'和'..'的文件
                $fname[$i++] = $filename;
            }
        }
       closedir($handler); //关闭目录
       $this->assign('fname',$fname)->display();
    }
